...
import { REQUEST_BODY_LIMIT, PROXY_API_KEY } from "./constants.js";
...
function parseError(error) {
  ...
  if (
    ...
    statusCode = 503;
    errorMessage =
      "Unable to connect to Antigravity API. Check that Antigravity is running.";
...
/**
 * OpenAI-compatible models endpoint (alias)
 */
...
app.post("/v1/chat/completions", async (req, res) => {
  try {
    ...
    logger.info(
      `[OpenAI API] Request for model: ${model}, stream: ${!!stream}`
    );
    ...
    if (stream) {
      ...
      try {
        ...
        for await (const event of sendMessageStream(
          ...
          if (event.type === "message_delta" && event.usage) {
            usage.output_tokens = event.usage.output_tokens || 0; // Usually cumulative or delta, but Anthropic sends total in delta
          }

          // Convert Anthropic event to OpenAI format
          const openaiChunk = convertAnthropicStreamToOpenAI(event, model);
        ...
        // Send final chunk with usage stats (OpenAI format)
        // If stream_options: { include_usage: true } is used (default in many new clients)
        const usageChunk = {
          ...
          usage: {
            ...
            // Include cache fields if non-zero (custom extension)
      ...
      const openaiResponse = convertAnthropicToOpenAI(response, model);
    ...
    logger.warn(
      `[OpenAI API] Returning error response: ${statusCode} ${errorType} - ${errorMessage}`
    );

    // Check if headers have already been sent (for streaming that failed mid-way)
    if (res.headersSent) {
      logger.warn(
        "[OpenAI API] Headers already sent, writing error as SSE event"
      );
      res.write(
        `data: ${JSON.stringify({
          error: {
...
// Endpoint /v1/messages removed as it is specific to Anthropic API
